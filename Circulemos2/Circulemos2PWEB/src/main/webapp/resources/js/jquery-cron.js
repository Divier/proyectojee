 ...
 *
 * Notes:
 * At this stage, we only support a subset de possible cron options.
...
(function($) {
    ...
    // options for minutes in an hora
    var str_opt_mih = "";
    for (var i = 0; i < 60; i++) {
        var j = (i < 10)? "0":"";
    ...
    // options for period
    var str_opt_period = "";
    var periods = ["minuto", "hora", "dia", "semana", "mes", "anio"];
    ...
    function getCronType(cron_str, opts) {
        ...
        // check format de initial cron value
    ...
    function getCurrentValue(c) {
        ...
        return [min, hora, dia, mes, dow].join(" ");
    }
    ...
    var methods = {
        init : function(opts) {
            ...
            block["dom"] = $("<span class='cron-block cron-block-dom'>"
                    + " el <select name='cron-dom'>" + str_opt_dom
                    + "</select> </span>")
                .appendTo(this)
                .data("root", this);
            ...
            block["mes"] = $("<span class='cron-block cron-block-mes'>"
                    + " de <select name='cron-mes'>" + str_opt_month
                    + "</select> </span>")
                .appendTo(this)
                .data("root", this);
            ...
            block["dow"] = $("<span class='cron-block cron-block-dow'>"
                    + " el <select name='cron-dow'>" + str_opt_dow
                    + "</select> </span>")
                .appendTo(this)
                .data("root", this);
